...
const matchHandler = (io, socket) => {
  ...
  socket.on('match:join', async ({ matchId }) => {
    try {
      ...
      socket.to(`match:${matchId}`).emit('match:opponent_status', {
...
// ── match:forfeit_request ─────────────────────────────────────
  ...
  // ── code:run ───────────────────────────────────────────────────
  // Runs code against PUBLIC test cases only — does NOT lock submission
  // Like LeetCode "Run Code" button
  socket.on('code:run', async ({ matchId, language, code }) => {
    try {
      ...
      if (!timer || timer.isExpired) {
        return socket.emit('match:error', { message: 'Time is up' });
      }

      socket.emit('match:run_started', {
        matchId,
        message: 'Running against sample test cases...',
      });

      // Fetch only PUBLIC test cases for run
      ...
      if (!problemResult.rows[0]) {
        return socket.emit('match:error', { message: 'Problem not found' });
      }
      ...
      if (publicTestCases.length === 0) {
        return socket.emit('match:run_result', {
          matchId,
          results: [],
      ...
      socket.emit('match:run_result', {
        matchId,
        results,
        passedCount,
        totalCount: results.length,
        message: `${passedCount}/${results.length} sample tests passed`,
      });
  ...
  // ── code:submit ────────────────────────────────────────────────
  // Submits against ALL test cases (public + hidden) — locks answer
  // Like LeetCode "Submit" button
  socket.on('code:submit', async ({ matchId, language, code }) => {
    try {
      ...
      if (!alreadySubmitted) {
        return socket.emit('match:error', { message: 'You have already submitted' });
      }

      socket.emit('match:submission_received', {
        matchId,
        message: 'Submission received. Evaluating against all test cases...',
      });
      ...
      socket.to(`match:${matchId}`).emit('match:opponent_status', {
        status: 'submitted',
        userId,
        message: 'Opponent has submitted their solution',
      });

      // Record submission time (used for tiebreaking only)
      ...
      // Get problem ID from match
      const matchResult = await pool.query(
        'SELECT problem_id FROM matches WHERE id = $1',
        [matchId]
      );
      const problemId = matchResult.rows[0]?.problem_id;

      if (!problemId) {
        logger.error('No problem assigned to match', { matchId });
        const ms = JSON.parse(await redisClient.get(`match:${matchId}:state`));
        if (ms) await refundBothPlayers(ms.playerAId, ms.playerBId, ms.entryFee, matchId);
        io.to(`match:${matchId}`).emit('match:error', {
          message: 'No problem assigned. Both players refunded.',
        });
        return;
      }

      // Notify client evaluation is running
      socket.emit('match:evaluating', {
        matchId,
        message: 'Running against all test cases including hidden ones...',
      });

      // Run real evaluation against ALL test cases
      const evalResult = await evaluateCode(matchId, userId, language, code, problemId);

      // Judge service failure — refund both
      if (evalResult.status === 'JUDGE_ERROR') {
        logger.error('Judge failed — refunding', { matchId });
        const ms = JSON.parse(await redisClient.get(`match:${matchId}:state`) || '{}');
        if (ms.playerAId) {
          ...
          io.to(`match:${matchId}`).emit('match:error', {
            message: 'Evaluation service error. Both players refunded.',
          });
        }
        return;
      }
      ...
      await redisClient.set(
        `match:${matchId}:score:${userId}`,
        evalResult.score,
        'EX', 7200
      );

      // Update submission in DB
      const judgeStatus = evalResult.score === 100 ? 'ACCEPTED'
        : evalResult.score > 0 ? 'PARTIAL'
        : 'WRONG_ANSWER';

      await pool.query(
        `UPDATE submissions
         SET status = $1, test_cases_passed = $2, test_cases_total = $3,
             execution_time_ms = $4
         WHERE match_id = $5 AND user_id = $6`,
        [judgeStatus, evalResult.passed, evalResult.total, evalResult.avgTimeMs, matchId, userId]
      );

      // Send evaluation result back to this player
      ...
      if (scoreA !== null && scoreB !== null) {
        // Both submitted — resolve immediately
        await determineAndResolveWinner(io, matchId, matchState);
      } else {
        // Still waiting for opponent
        socket.emit('match:waiting_opponent', {
          matchId,
  ...
  socket.on('match:leave', async ({ matchId }) => {
    ...
    socket.to(`match:${matchId}`).emit('match:opponent_status', {
      status: 'disconnected', userId,
    });
  });
};

// ── Winner determination ───────────────────────────────────────────
// SCORE IS ALWAYS PRIMARY — time is only a tiebreaker when scores are equal
const determineAndResolveWinner = async (io, matchId, matchState) => {
  ...
  let winnerId, loserId, winReason;

  // RULE 1: Higher score wins — always
  if (scoreA > scoreB) {
    winnerId = matchState.playerAId;
    loserId = matchState.playerBId;
    winReason = `Higher score (${scoreA}% vs ${scoreB}%)`;
  } else if (scoreB > scoreA) {
    winnerId = matchState.playerBId;
    loserId = matchState.playerAId;
    winReason = `Higher score (${scoreB}% vs ${scoreA}%)`;
  } else {
    // RULE 2: Equal scores — faster submission wins
    if (timeA <= timeB) {
      winnerId = matchState.playerAId;
      loserId = matchState.playerBId;
      winReason = `Equal score (${scoreA}%), faster submission`;
    } else {
      winnerId = matchState.playerBId;
      loserId = matchState.playerAId;
      winReason = `Equal score (${scoreB}%), faster submission`;
    }
  }

  logger.info('Winner determined', {
    matchId, winnerId, scoreA, scoreB, timeA, timeB, winReason,
  });
...
const resolveMatch = async (io, matchId, winnerId, loserId, resolveType = 'COMPLETED') => {
  try {
    ...
    const timeA = parseInt(
      await redisClient.get(`match:${matchId}:time:${matchState.playerAId}`) || '0'
    );
    const timeB = parseInt(
      await redisClient.get(`match:${matchId}:time:${matchState.playerBId}`) || '0'
    );
    ...
    await pool.query(
      `UPDATE matches
       SET status = 'COMPLETED', winner_id = $1,
           ...
           player_a_time_ms = $4, player_b_time_ms = $5,
           completed_at = NOW()
       WHERE id = $6`,
    ...
    await pool.query(
      `UPDATE users SET matches_played = matches_played + 1,
       ...
       skill_rating = skill_rating + 25
       WHERE id = $1`,
      [winnerId]
    );
    await pool.query(
      `UPDATE users SET matches_played = matches_played + 1,
       skill_rating = GREATEST(skill_rating - 15, 0)
       WHERE id = $1`,
      [loserId]
    );
    ...
    io.to(`match:${matchId}`).emit('match:result', {
      ...
      platformFee,
      scores: {
        [matchState.playerAId]: scoreA,
        [matchState.playerBId]: scoreB,
      },
      times: {
        [matchState.playerAId]: timeA,
        [matchState.playerBId]: timeB,
      },
      winnerNewBalance: walletResult.rows[0]?.balance || 0,
    });

    logger.info('Match resolved', { matchId, winnerId, loserId, netPrize, resolveType });
    ...
    try {
      ...
      if (matchStateRaw) {
        ...
        await refundBothPlayers(
          matchState.playerAId, matchState.playerBId,
          matchState.entryFee, matchId
        );
        io.to(`match:${matchId}`).emit('match:error', {
          message: 'Match resolution failed. Both players refunded.',
        });
      }
    } catch (refundErr) {
      logger.error('Refund also failed', { matchId, error: refundErr.message });
    }
  }
};
